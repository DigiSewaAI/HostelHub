...
<div class="bg-white rounded-2xl shadow-sm p-6">
    ...
    <div class="mb-6 bg-gray-50 p-4 rounded-xl border border-gray-200">
        <form method="GET" action="{{ route('owner.galleries.index') }}" class="flex flex-wrap gap-4 items-center">
        ...
        <div class="bg-blue-50 p-4 rounded-xl border border-blue-200">
            <div class="flex justify-between items-center">
                <div>
                    ...
                    <p class="text-2xl font-bold text-blue-800">{{ $galleries->total() }}</p>
                </div>
        ...
        <div class="bg-purple-50 p-4 rounded-xl border border-purple-200">
            <div class="flex justify-between items-center">
                <div>
                    ...
                    <p class="text-2xl font-bold text-purple-800">{{ $galleries->where('media_type', 'photo')->count() }}</p>
                </div>
        ...
        <div class="bg-orange-50 p-4 rounded-xl border border-orange-200">
            <div class="flex justify-between items-center">
                <div>
                    ...
                    <p class="text-2xl font-bold text-orange-800">{{ $galleries->whereIn('media_type', ['local_video', 'external_video'])->count() }}</p>
                </div>
            ...
            <div class="bg-white rounded-xl border border-gray-200 overflow-hidden transition-all duration-300 hover:shadow-lg hover:border-blue-300 gallery-card group">
                ...
                <div class="relative aspect-video bg-gray-100 overflow-hidden">
                    @if($gallery->media_type === 'photo')
                        <!-- Image Display -->
                        <div class="w-full h-full cursor-pointer gallery-item" data-type="image" data-url="{{ $gallery->media_url }}">
                    ...
                    @elseif($gallery->media_type === 'external_video')
                        <!-- YouTube Video -->
                        <div class="w-full h-full cursor-pointer gallery-item" data-type="youtube" data-url="{{ $gallery->external_link }}">
                            <img src="{{ $gallery->thumbnail_url }}" 
                                 alt="{{ $gallery->title }}"
                    ...
                    @elseif($gallery->media_type === 'local_video')
                        ...
                        <div class="w-full h-full cursor-pointer gallery-item" data-type="video" data-url="{{ $gallery->media_url }}">
                            @if($gallery->thumbnail && Storage::disk('public')->exists($gallery->thumbnail))
                                <img src="{{ asset('storage/' . $gallery->thumbnail) }}" 
                                     alt="{{ $gallery->title }}"
                                     class="w-full h-full object-cover transition-transform duration-300 group-hover:scale-105">
                            @elseif($gallery->file_path && Storage::disk('public')->exists($gallery->file_path))
                ...
                <!-- Content Section -->
                <div class="p-4">
                    <h3 class="font-semibold text-gray-800 text-sm mb-2 nepali leading-tight line-clamp-2">{{ $gallery->title }}</h3>
                    
                    @if($gallery->description)
                        <p class="text-gray-600 text-xs mb-3 nepali leading-relaxed line-clamp-2">{{ $gallery->description }}</p>
                    @endif

                    <div class="flex justify-between items-center text-xs text-gray-500 mb-3">
                        <span class="nepali flex items-center">
                            <i class="fas fa-calendar mr-1"></i> {{ $gallery->created_at->format('Y-m-d') }}
                        </span>
                        <span class="flex items-center {{ $gallery->is_active ? 'text-green-600' : 'text-red-600' }}">
                    ...
                    <div class="flex justify-between items-center pt-3 border-t border-gray-100 action-buttons">
                        <div class="flex space-x-1">
                            ...
                            <form action="{{ route('owner.galleries.toggle-featured', $gallery) }}" method="POST" class="inline">
                                ...
                                @method('PATCH')
                                <button type="submit" 
                                        class="text-white {{ $gallery->is_featured ? 'bg-yellow-500 hover:bg-yellow-600' : 'bg-gray-400 hover:bg-gray-500' }} transition-colors duration-200 p-1.5 rounded-lg text-sm"
                                        title="{{ $gallery->is_featured ? 'फिचर्ड हटाउनुहोस्' : 'फिचर्ड बनाउनुहोस्' }}">
                                    <i class="fas fa-star"></i>
                                </button>
                            </form>
                            ...
                            <form action="{{ route('owner.galleries.toggle-active', $gallery) }}" method="POST" class="inline">
                                ...
                                @method('PATCH')
                                <button type="submit" 
                                        class="text-white {{ $gallery->is_active ? 'bg-green-500 hover:bg-green-600' : 'bg-gray-400 hover:bg-gray-500' }} transition-colors duration-200 p-1.5 rounded-lg text-sm"
                                        title="{{ $gallery->is_active ? 'निष्क्रिय बनाउनुहोस्' : 'सक्रिय बनाउनुहोस्' }}">
                        ...
                        <div class="flex space-x-1">
                            <!-- Show Button -->
                            <a href="{{ route('owner.galleries.show', $gallery) }}" 
