...
<div class="container mx-auto px-4 py-6">
    <div class="max-w-3xl mx-auto bg-white rounded-lg shadow-md p-6">
        ...
        <form action="{{ route('owner.meal-menus.update', $mealMenu) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div>
                    ...
                    <select name="day_of_week" id="day_of_week" class="w-full border border-gray-300 rounded-md p-2 focus:ring-indigo-500 focus:border-indigo-500" required>
                        ...
                        <option value="sunday" {{ $mealMenu->day_of_week == 'sunday' ? 'selected' : '' }}>आइतबार</option>
                        <option value="monday" {{ $mealMenu->day_of_week == 'monday' ? 'selected' : '' }}>सोमबार</option>
                        <option value="tuesday" {{ $mealMenu->day_of_week == 'tuesday' ? 'selected' : '' }}>मंगलबार</option>
                        <option value="wednesday" {{ $mealMenu->day_of_week == 'wednesday' ? 'selected' : '' }}>बुधबार</option>
                        <option value="thursday" {{ $mealMenu->day_of_week == 'thursday' ? 'selected' : '' }}>बिहिबार</option>
                        <option value="friday" {{ $mealMenu->day_of_week == 'friday' ? 'selected' : '' }}>शुक्रबार</option>
                        <option value="saturday" {{ $mealMenu->day_of_week == 'saturday' ? 'selected' : '' }}>शनिबार</option>
                    </select>
                </div>
            ...
            <!-- FIXED: Dynamic items input with existing data -->
            <div class="mb-6">
                <label class="block text-sm font-medium text-gray-700 mb-1">खानाका वस्तुहरू <span class="text-red-500">*</span></label>
                <div id="items-container">
                    @if(is_array($mealMenu->items) && count($mealMenu->items) > 0)
                        @foreach($mealMenu->items as $index => $item)
                            <div class="flex items-center gap-2 mb-2">
                                <input type="text" name="items[]" class="w-full border border-gray-300 rounded-md p-2 focus:ring-indigo-500 focus:border-indigo-500" required value="{{ $item }}">
                                @if($index === 0)
            ...
            <div class="mb-6">
                ...
                <textarea name="description" id="description" rows="3" class="w-full border border-gray-300 rounded-md p-2 focus:ring-indigo-500 focus:border-indigo-500" placeholder="अतिरिक्त विवरण...">{{ $mealMenu->description }}</textarea>
            </div>
            ...
            <div class="mb-6">
                ...
                @if($mealMenu->image)
                    <div class="mt-2">
                        ...
                        <p class="text-sm text-gray-500 mt-1">हालको तस्बिर</p>
                    </div>
                @endif
            </div>
            ...
            <div class="mb-6">
                <label class="flex items-center">
                    <input type="checkbox" name="is_active" value="1" {{ $mealMenu->is_active ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
