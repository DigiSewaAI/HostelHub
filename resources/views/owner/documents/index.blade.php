@extends('layouts.owner')

@section('title', 'कागजात व्यवस्थापन')

@section('content')
<div class="container-fluid">
    <div class="row">
        <div class="col-12">
            <div class="card shadow">
                <div class="card-header bg-primary text-white py-3">
                    <div class="d-flex justify-content-between align-items-center">
                        <h5 class="mb-0">
                            <i class="fas fa-file-alt me-2"></i>कागजातहरू
                        </h5>
                        <div>
                            <a href="{{ route('owner.documents.create') }}" class="btn btn-light btn-sm">
                                <i class="fas fa-plus me-1"></i>नयाँ कागजात
                            </a>
                            <a href="{{ route('owner.documents.search') }}" class="btn btn-light btn-sm">
                                <i class="fas fa-search me-1"></i>खोज्नुहोस्
                            </a>
                        </div>
                    </div>
                </div>
                
                <div class="card-body">
                    <!-- Search and Filter Form -->
                    <form action="{{ route('owner.documents.index') }}" method="GET" class="mb-4">
                        <div class="row g-3">
                            <div class="col-md-4">
                                <input type="text" name="search" class="form-control" 
                                       placeholder="विद्यार्थीको नाम वा कागजातको नाम..." 
                                       value="{{ request('search') }}">
                            </div>
                            <div class="col-md-3">
                                <select name="document_type" class="form-select">
                                    <option value="">सबै प्रकार</option>
                                    @foreach($documentTypes as $key => $type)
                                        <option value="{{ $key }}" {{ request('document_type') == $key ? 'selected' : '' }}>
                                            {{ $type }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-md-2">
                                <input type="date" name="start_date" class="form-control" 
                                       value="{{ request('start_date') }}" placeholder="सुरु मिति">
                            </div>
                            <div class="col-md-2">
                                <input type="date" name="end_date" class="form-control" 
                                       value="{{ request('end_date') }}" placeholder="अन्त्य मिति">
                            </div>
                            <div class="col-md-1">
                                <button type="submit" class="btn btn-primary w-100">
                                    <i class="fas fa-filter"></i>
                                </button>
                            </div>
                        </div>
                    </form>

                    @if($documents->count() > 0)
                        <div class="table-responsive">
                            <table class="table table-bordered table-hover">
                                <thead class="table-light">
                                    <tr>
                                        <th>#</th>
                                        <th>विद्यार्थी</th>
                                        <th>कागजातको प्रकार</th>
                                        <th>कागजातको नाम</th>
                                        <th>फाइल साइज</th>
                                        <th>अपलोड मिति</th>
                                        <th>म्याद</th>
                                        <th>कार्यहरू</th>
                                    </tr>
                                </thead>
                                <tbody>
                                    @foreach($documents as $document)
                                    <tr>
                                        <td>{{ $loop->iteration }}</td>
                                        <td>{{ $document->student->user->name ?? 'N/A' }}</td>
                                        <td>
                                            <span class="badge bg-info">
                                                {{ $document->getDocumentTypeNepaliAttribute() }}
                                            </span>
                                        </td>
                                        <td>{{ $document->original_name }}</td>
                                        <td>{{ number_format($document->file_size / 1024, 2) }} KB</td>
                                        <td>{{ $document->created_at->format('Y-m-d') }}</td>
                                        <td>
                                            @if($document->expiry_date)
                                                @if($document->is_expired)
                                                    <span class="badge bg-danger">म्याद नाघेको</span>
                                                @else
                                                    <span class="badge bg-warning">{{ $document->expiry_date->format('Y-m-d') }}</span>
                                                @endif
                                            @else
                                                <span class="badge bg-secondary">म्याद नभएको</span>
                                            @endif
                                        </td>
                                        <td>
                                            <div class="btn-group btn-group-sm">
                                                <a href="{{ route('owner.documents.show', $document) }}" 
                                                   class="btn btn-info" title="हेर्नुहोस्">
                                                    <i class="fas fa-eye"></i>
                                                </a>
                                                <a href="{{ route('owner.documents.download', $document) }}" 
                                                   class="btn btn-success" title="डाउनलोड गर्नुहोस्">
                                                    <i class="fas fa-download"></i>
                                                </a>
                                                @if($document->uploaded_by == auth()->id())
                                                <form action="{{ route('owner.documents.destroy', $document) }}" 
                                                      method="POST" class="d-inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="btn btn-danger" 
                                                            onclick="return confirm('के तपाइँ यो कागजात मेटाउन निश्चित हुनुहुन्छ?')"
                                                            title="मेटाउनुहोस्">
                                                        <i class="fas fa-trash"></i>
                                                    </button>
                                                </form>
                                                @endif
                                            </div>
                                        </td>
                                    </tr>
                                    @endforeach
                                </tbody>
                            </table>
                        </div>
                        
                        <!-- Pagination -->
                        <div class="d-flex justify-content-center mt-4">
                            {{ $documents->links() }}
                        </div>
                    @else
                        <div class="text-center py-5">
                            <i class="fas fa-folder-open fa-3x text-muted mb-3"></i>
                            <h5 class="text-muted">कुनै कागजात भेटिएन</h5>
                            <p class="text-muted">कृपया पहिलो कागजात अपलोड गर्नुहोस्</p>
                            <a href="{{ route('owner.documents.create') }}" class="btn btn-primary">
                                <i class="fas fa-plus me-1"></i>नयाँ कागजात अपलोड गर्नुहोस्
                            </a>
                        </div>
                    @endif
                </div>
            </div>
        </div>
    </div>
</div>
@endsection