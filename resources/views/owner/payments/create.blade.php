...
<div class="container mx-auto px-4 py-6">
    <div class="max-w-3xl mx-auto bg-white rounded-lg shadow-md p-6">
        <div class="flex justify-between items-center mb-6">
            <h1 class="text-2xl font-bold text-gray-800">नयाँ भुक्तानी थप्नुहोस्</h1>
            <a href="{{ route('owner.payments.index') }}" class="text-indigo-600 hover:text-indigo-900 flex items-center">
        ...
        <form action="{{ route('owner.payments.store') }}" method="POST">
                ...
                <div>
                    ...
                    <select name="student_id" id="student_id" class="w-full border border-gray-300 rounded-md p-2 focus:ring-indigo-500 focus:border-indigo-500" required>
                        ...
                        @foreach($students as $student)
                            <option value="{{ $student->id }}">{{ $student->name }} ({{ $student->room->room_number ?? 'N/A' }})</option>
                        @endforeach
                    </select>
                    @error('student_id')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <select name="payment_method" id="payment_method" class="w-full border border-gray-300 rounded-md p-2 focus:ring-indigo-500 focus:border-indigo-500" required>
                        ...
                        <option value="bank_transfer" {{ old('payment_method') == 'bank_transfer' ? 'selected' : '' }}>बैंक ट्रान्सफर</option>
                        <option value="online" {{ old('payment_method') == 'online' ? 'selected' : '' }}>अनलाइन</option>
                    </select>
            ...
            <div class="mb-6">
                ...
                <select name="status" id="status" class="w-full border border-gray-300 rounded-md p-2 focus:ring-indigo-500 focus:border-indigo-500" required>
                    <option value="pending" {{ old('status') == 'pending' ? 'selected' : '' }}>प्रतीक्षामा</option>
                    <option value="completed" {{ old('status') == 'completed' ? 'selected' : '' }}>पूरा भएको</option>
