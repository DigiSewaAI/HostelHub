...
use App\Http\Controllers\Admin\RoomController as AdminRoomController; // ✅ ADDED: Admin RoomController alias
use App\Http\Controllers\RoomController; // ✅ KEPT: General RoomController
...
// ✅ EMERGENCY: Debug publish/unpublish functionality
...
// ✅ FIXED: Admin routes with proper middleware order - COMPLETELY REMOVE hasOrganization
Route::prefix('admin')
    ->middleware(['auth', 'role:admin']) // ✅ ONLY auth and admin role - NO hasOrganization
    ->group(function () {
...
// ❌ REMOVED: Problematic RoomController route - this should be in owner.php
...
// ✅ TEMPORARY: GET version for testing
Route::get('/test-publish-get/{hostelId}', function ($hostelId) {
    ...
    return redirect()->route('admin.hostels.index')
        ->with('success', 'होस्टल सफलतापूर्वक प्रकाशित गरियो (GET Test)');
})->middleware(['auth']);

// ✅ ADD THESE SINGLE HOSTEL PUBLISH ROUTES
...
// ✅ EMERGENCY: Simple publish for any hostel
Route::get('/quick-publish/{hostelId}', function ($hostelId) {
    ...
    return redirect()->route('admin.hostels.index')
        ->with('success', 'होस्टल सफलतापूर्वक प्रकाशित गरियो (Quick Fix)');
})->middleware(['auth']);
